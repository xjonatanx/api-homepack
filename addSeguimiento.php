...
if($resultedsignature == $recieved_signature) {
    ...
    if(isset($_FILES['foto'])) {
        ...
        $image_route = "/img/".$id_unica.$filename;
        
        // Check extension
        if(in_array(strtolower($extension),$valid_extensions) ) {
           // Upload foto
           if(move_uploaded_file($_FILES['foto']['tmp_name'], "img/".$id_unica.$filename)){
                $estado = $_POST["estado"];
                $detalle = $_POST["detalle"];
                $fk_id_despacho = $_POST["fk_id_despacho"];
                $nombre = $_POST["nombre_recepcion"];
                $apellido = $_POST["apellido_recepcion"];
                $rut = $_POST["rut_recepcion"];
                
                $sql = "INSERT INTO seguimiento (fecha, estado, detalle, foto, fk_id_despacho, nombre_recepcion, apellido_recepcion, rut_recepcion) VALUES (NOW(), '$estado', '$detalle', '$image_route', $fk_id_despacho, '$nombre', '$apellido', '$rut')";
                
                if ($conn->query($sql) === TRUE) {
                    sendMail($fk_id_despacho, $estado, $detalle, $image_route);
                    $myObj->status = 1;
                    $myObj->mesagge = "Seguimiento ingresado correctamente.";
                    $myJSON = json_encode($myObj);
                    echo $myJSON;
                } else {
                    $myObj->status = 0;
                    $myObj->mesagge = "Ocurrió un problema al ingresar el seguimiento.";
                    $myJSON = json_encode($myObj);
                    echo $myJSON;
                } 
                $conn->close();
                ...
                $myObj->status = 0;
                $myObj->mesagge = "Ocurrió un problema al subir la foto.";
                $myJSON = json_encode($myObj);
                echo $myJSON;
           }
        }else{
            $myObj->status = 0;
            $myObj->mesagge = "La extención de la fotografía no es válida.";
            $myJSON = json_encode($myObj);
            echo $myJSON;
        }
    } else {
        ...
        $sql = "INSERT INTO seguimiento (fecha, estado, detalle, foto, fk_id_despacho, nombre_recepcion, apellido_recepcion, rut_recepcion) VALUES (NOW(), '$estado', '$detalle', null, $fk_id_despacho, '$nombre', '$apellido', '$rut')";
        
        if ($conn->query($sql) === TRUE) {
            sendMail($fk_id_despacho, $estado, $detalle, null);
            $myObj->status = 1;
            $myObj->mesagge = "Seguimiento ingresado correctamente.";
            $myJSON = json_encode($myObj);
            echo $myJSON;
...
function sendMail($id_despacho, $estado, $detalle, $foto)
{   
    if($detalle != "") {
        $detalle = '<b>Observaciones:</b><br>'. $detalle .'<br><br>';
    }
    $text_foto = "";
    ...
    if (mysqli_num_rows($result) > 0) {
        while($row = mysqli_fetch_assoc($result)) {
            ...
            try {
                ...
                $mail->addAddress($row["email_remitente"], $row["nombre_remitente"]);     // Add a recipient
                $mail->addAddress($row["email_destinatario"], $row["nombre_destinatario"]);     // Add a recipient
                if(isset($foto)) {
                ...
                $mail->Body    = utf8_decode('
                    ...
                    <!--[if mso]><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr style="line-height:0px"><td style="padding-right: 40px;padding-left: 40px;" align="center"><![endif]--><img align="center" alt="Im an image" border="0" class="center fixedwidth" src="https://yitp.cl/img_hpchile/camion-de-reparto.png" style="text-decoration: none; -ms-interpolation-mode: bicubic; height: auto; border: 0; width: 100%; max-width: 160px; display: block;" title="Im an image" width="160"/>
                    <!--[if mso]></td></tr></table><![endif]-->
                    </div>
                    ...
                    <br>Estimado usuario /a<br><br>Junto con saludar, le informamos que el estado de su paquete número '. $row["numero_seguimiento"] .' ha sido actualizado a <b>'. $estado .'</b>.<br><br>'.$detalle.''. $text_foto .'
                    </span></p>
                    </div>
                    </div>
                    <!--[if mso]></td></tr></table><![endif]-->
