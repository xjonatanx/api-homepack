    ...
    if($resultedsignature == $recieved_signature) {
        ...
        if($perfil == "admin") {
            ...
            $count = $conn->query("
                ...
                WHERE despachos.sobrancia = 1 AND (usuarios.nombre LIKE '%$search%' OR despachos.orden_compra LIKE '%$search%' OR despachos.numero_seguimiento LIKE '%$search%');");
        ...
        if($perfil == "chofer") {
            $search = $_GET['search'];
            $count = $conn->query("SELECT COUNT(*) AS countDespachos FROM despachos WHERE sobrancia = 1 AND id_chofer = $id_chofer AND (orden_compra LIKE '%$search%' OR numero_seguimiento LIKE '%$search%');");
            $fila = $count->fetch_assoc();
            $total = $fila['countDespachos'];
            ...
            LEFT JOIN usuarios
            ON despachos.id_chofer = usuarios.id
            WHERE despachos.sobrancia = 1 AND despachos.id_chofer = $id_chofer AND (despachos.orden_compra LIKE '%$search%'  OR despachos.numero_seguimiento LIKE '%$search%') 
            ORDER BY fecha_ingreso DESC
            LIMIT $offset, $no_of_records_per_page;");
        }
