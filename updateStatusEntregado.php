...
if($resultedsignature == $recieved_signature) {
    if(isset($_FILES['foto'])){
        ...
        if(in_array($file_ext,$extensions)=== false){
            $errors[]="Extención del archivo no permitida, por favor seleccione un archivo JPEG o PNG.";
        }
          
        if($file_size > 5097152){
            $errors[]='Archivo excede el tamaño máximo 5 MB';
        }
          
        if(empty($errors)==true){
            move_uploaded_file($file_tmp,"img/".$id_unica.$file_name);
            $id = $_POST["id"];
            ...
            if ($conn->query($sql) === TRUE) {
                ...
                if ($result->num_rows > 0) {
                  while($row = $result->fetch_assoc()) {
                        $image_route = "/img/".$id_unica.$file_name;
                        $estado = "ENTREGADO";
                        $fk_id_despacho = $row["id"];
                        $sql = "INSERT INTO seguimiento (fecha, estado, foto, fk_id_despacho, nombre_recepcion, apellido_recepcion, rut_recepcion) VALUES (NOW(), '$estado', '$image_route', $fk_id_despacho, '$nombre', '$apellido', '$rut')";
                        $conn -> query($sql);
                        sendMail($fk_id_despacho, $estado, $image_route);
                    }
                }
                $myObj->status = 1;
