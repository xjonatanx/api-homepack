    ...
    if ($conn->query($sql) === TRUE) {
        ...
        if ($result->num_rows > 0) {
          while($row = $result->fetch_assoc()) {
                $estado = "ACTUALIZADO A SOBRANCIAS";
                $fk_id_despacho = $row["id"];
                $sql = "INSERT INTO seguimiento (fecha, estado, foto, fk_id_despacho, nombre_recepcion, apellido_recepcion, rut_recepcion, motivo) VALUES (NOW(), '$estado', null, $fk_id_despacho, null, null, null, '$motivo')";
                $conn -> query($sql);
                sendMail($fk_id_despacho, $estado);
            }
        }
        $myObj->status = 1;
        $myObj->mesagge = "El despacho se ha movido a sobrancias correctamente.";
